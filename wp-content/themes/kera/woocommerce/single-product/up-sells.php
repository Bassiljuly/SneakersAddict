...
$rows = apply_filters( 'kera_supermaket_woo_row_single_full', false,2 ); 
if($rows) {
	$rows = 4;
...
$show_product_upsells = kera_tbay_get_config('enable_product_upsells', true);

if ( $upsells && $show_product_upsells ) : ?>

	<div class="upsells tbay-element tbay-element-products">
		<?php 
			$heading = apply_filters( 'woocommerce_product_upsells_products_heading', esc_html__( 'You may also like&hellip;', 'kera' ) );
		if ( $heading ) : ?>
