		...
		<div class="mcart-border">
							...
							<li id="mcitem-<?php echo esc_attr($_id);?>-<?php echo esc_attr($cart_item_key); ?>">
								...
								<div class="product-details">
									...
									<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
									    '<a href="%s" class="remove remove_from_cart_button" aria-label="%s" data-product_id="%s" data-product_sku="%s" data-cart_item_key="%s"><i class="icon-trash"></i></a>',
			...
			<?php else: ?>
				<ul class="cart_empty <?php echo esc_attr($args['list_class']); ?>">
					<li><span><?php esc_html_e('No Products In The Cart','kera') ?></span></li>
					<li class="total"><a class="button wc-continue" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>"><?php esc_html_e( 'start shopping', 'kera' ) ?><i class="icon-arrow-right"></i></a></li>
				</ul>
			<?php endif; ?>
			...
			<?php if(sizeof(WC()->cart->get_cart()) > 0) : ?>
				<div class="group-button">
					...
					<p class="buttons">
						<a href="<?php echo esc_url( wc_get_checkout_url() );?>" class="button checkout"><?php esc_html_e('Checkout', 'kera'); ?></a>
						<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="button view-cart"><?php esc_html_e('View Cart', 'kera'); ?></a>
					</p>
				</div>
			<?php endif; ?>
