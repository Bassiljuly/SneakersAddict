...
if ( !class_exists("Kera_Tbay_mmenu_menu") ) {
    class Kera_Tbay_mmenu_menu extends Walker_Nav_Menu {
       ...
       public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->tbay_text_label) && !empty($item->tbay_text_label) ) {
                switch ( $item->tbay_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'kera');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'kera');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Sale', 'kera');
                        break;
        ...
        public function generateSubMegaMenu( $item, $tbay_mega_profile ) {
            ...
            if ( $tbay_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    ...
                    if( defined('TBAY_ELEMENTOR_ACTIVED') && kera_is_elementor_activated() && in_array( 'tbay_megamenu', get_option( 'elementor_cpt_support' ) ) ) {
                        $content = Elementor\Plugin::instance()->frontend->get_builder_content_for_display( $post->ID, kera_get_elementor_css_print_method() );
                    } else {
                    ...
                    if( kera_tbay_get_config('ajax_dropdown_megamenu', false) ) {  
