...
if ( !function_exists('kera_tbay_woocommerce_get_cookie_display_mode') ) {
    function kera_tbay_woocommerce_get_cookie_display_mode() {
        ...
        if( isset($_COOKIE['kera_display_mode']) && $_COOKIE['kera_display_mode'] == 'grid' ) {
            $woo_mode = 'grid';
        } else if ( isset($_COOKIE['kera_display_mode']) && $_COOKIE['kera_display_mode'] == 'grid2' ) {
            $woo_mode = 'grid2';
        } else if( isset($_COOKIE['kera_display_mode']) && $_COOKIE['kera_display_mode'] == 'list' ) {
            $woo_mode = 'list';
        }
...
if( ! function_exists( 'kera_swatches_list' ) ) {
    function kera_swatches_list( $attribute_name = false ) {
        ...
        if( ! $attribute_name ) {
            $attribute_name = kera_get_swatches_attribute();
        }
        ...
        <form method="get" class="woocommerce-fillter">
            <select name="<?php echo esc_attr($name); ?>" onchange="this.form.submit()" class="select">
                <?php $i = 0; foreach( $options as $key => $value ) : ?>
                    <option class="<?php echo (!empty($class[$i])) ? trim($class[$i]) : '';?>" value="<?php echo esc_attr( $key ); ?>" <?php selected( $key, kera_woocommerce_get_fillter($name, $default) ); ?> ><?php echo trim($value);?></option>
                    <?php $i++; ?>
                <?php endforeach; ?>
            </select>
        <?php
    ...
    function kera_the_yith_compare($product_id) {
        ...
        if( class_exists( 'YITH_Woocompare' ) ) { ?>
            ...
            <div class="yith-compare">
                <a href="<?php echo wp_nonce_url( add_query_arg( $url_args ), $action_add ); ?>" title="<?php esc_attr_e('Compare', 'kera'); ?>" class="compare" data-product_id="<?php echo esc_attr($product_id); ?>">
                    <span><?php esc_html_e('Compare', 'kera'); ?></span>
                </a>
            </div>
        <?php } 
...
if ( !function_exists('kera_product_quickview_image') ) {
    function kera_product_quickview_image() {
 ...
 */
if ( !function_exists('kera_override_woocommerce_output_product_data_tabs') ) {
  function woocommerce_output_product_data_tabs() {

     if( wp_is_mobile() && kera_tbay_get_config('enable_tabs_mobile', false) ) {
      ...
      if( $tabs_layout !== 'fulltext' ) {
        ...
        add_filter('woocommerce_product_more_product_heading', '__return_empty_string', 10, 1);
      }
...
if (!function_exists('kera_tbay_display_custom_tab_builder') ) {
  function kera_tbay_display_custom_tab_builder($tabs) {
    global $tabs_builder;
    $tabs_builder = true;
    $args = array(
      'name'        => $tabs,
...
if ( ! function_exists( 'kera_get_products_tab_ajax' ) ) {
	function kera_get_products_tab_ajax() {
		if ( ! empty( $_POST['atts'] ) ) {
            ...
            $product_type           = kera_clean( $_POST['value'] );
