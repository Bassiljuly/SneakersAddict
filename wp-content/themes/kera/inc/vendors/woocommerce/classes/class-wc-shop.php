...
if ( ! class_exists( 'Kera_Shop_WooCommerce' ) ) :
	...
	class Kera_Shop_WooCommerce  {
		...
		public function __construct() {
			...
			add_filter( 'kera_woo_subcat', array( $this, 'subcat_archives'), 10, 1 );
		...
		public function shop_per_page() {
			...
			if( isset($_GET['product_per_page']) && is_numeric($_GET['product_per_page']) ) {
	            ...
	            $value = kera_tbay_get_config('number_products_per_page', 12);          
	        }
		...
		public function shop_columns() {
			...
			if( isset($_GET['product_columns']) && is_numeric($_GET['product_columns']) ) {
	          ...
	          $value = kera_tbay_get_config('product_columns', 4);          
	        }
		...
		public function shop_load_more() {
	        ...
	        if((strpos($category, ',') !== false )) {
	            ...
	            $loop = kera_tbay_get_products( $categories, $type , $paged, $number_load, '', '', $number, $offset );
	        } else {

	            if( $category == -1 ) {
	                $loop = kera_tbay_get_products( '', $type , $paged, $number_load, '', '', $number, $offset );
	            } else {
	              $loop = kera_tbay_get_products( array($category), '' , $paged, $number_load, '', '', $number, $offset );  
	            } 
		...
		public function subcat_archives() {
