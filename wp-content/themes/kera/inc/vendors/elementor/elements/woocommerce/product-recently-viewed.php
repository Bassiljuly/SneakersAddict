...
if ( ! defined( 'ABSPATH' ) || function_exists('Kera_Elementor_Product_Recently_Viewed') ) {
...
class Kera_Elementor_Product_Recently_Viewed extends Kera_Elementor_Carousel_Base {
    ...
    public function get_title() {
        return esc_html__( 'Kera Product Recently Viewed', 'kera' );
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'empty',
            [
                'label' => esc_html__( 'Empty Result - Custom Paragraph', 'kera' ),
                'type' => Controls_Manager::TEXTAREA,
                'default' => esc_html__('You have no recently viewed item.', 'kera'), 
            ]
        );
    ...
    protected function register_control_style_header_title() {
        ...
        $this->add_control(
            ...
            [
                'label' => esc_html__( 'Hover Background', 'kera' ),
                'type' => Controls_Manager::COLOR,
                'default' => '',
                'selectors' => [
    ...
    private function register_control_header() {
        ...
        $this->add_control(
            'icon_header_recently',
            [
                'label' => esc_html__('Choose Icon', 'kera'),
                'type' => Controls_Manager::ICONS,
				'default' => [
                    'value' => 'zmdi zmdi-replay',
        ...
        $this->add_control(
            ...
            [
                ...
                'type' => Controls_Manager::TEXT,
                'default' => esc_html__('Recently Viewed', 'kera'),
                'label_block' => true,
                'condition' => [
                    'position_displayed' => 'header'
                ],
        ...
        $this->add_control(
            'header_column',
            [
                'label'     => esc_html__('Columns and max item', 'kera'),
                'type'      => \Elementor\Controls_Manager::SELECT,
                'default'   => 8,
                'separator'    => 'after',
                'condition' => [
    ...
    public function get_max_columns() {
        ...
        $value = apply_filters( 'kera_admin_elementor_recently_viewed_header_columns', [
           4 => 4,
           5 => 5,
           6 => 6,
           7 => 7,
    ...
    private function get_recently_viewed( $limit ) {
        ...
        $args = kera_tbay_get_products_recently_viewed($limit);
        $args = apply_filters( 'kera_list_recently_viewed_products_args', $args );
    ...
    public function render_content_header() {
        ...
        $empty                      =  esc_html__( 'You have no recent viewed item.', 'kera' );
        ...
        <h3 class="header-title">       
            <?php
                $this->render_icon_header(); 
                echo trim($header_title); 
            ?>
        </h3>
    ...
    private function render_btn_readmore($count) {
        $settings = $this->get_settings_for_display();
        extract($settings);
        $products_list              =  kera_tbay_wc_track_user_get_cookie();
        ...
        if( $enable_readmore && ($all > intval($count) ) && !empty($link) ) : ?>
            <a class="btn-readmore" href="<?php echo esc_url($link); ?>" title="<?php esc_attr( $readmore_text ); ?>"><?php echo trim($readmore_text); ?></a>
        <?php endif;
    }
    public function render_icon_header() {
        $settings = $this->get_settings();
        extract( $settings );
        if( empty( $icon_header_recently['value'] ) ) return;
       
        $this->add_render_attribute( 'icon', 'class', $icon_header_recently['value'] );
...
$widgets_manager->register_widget_type(new Kera_Elementor_Product_Recently_Viewed());