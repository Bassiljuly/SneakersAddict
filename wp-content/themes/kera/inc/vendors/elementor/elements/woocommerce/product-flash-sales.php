...
if ( ! defined( 'ABSPATH' ) || function_exists('Kera_Elementor_Product_Flash_Sales') ) {
...
class Kera_Elementor_Product_Flash_Sales extends Kera_Elementor_Carousel_Base {
    ...
    public function get_title() {
        return esc_html__( 'Kera Product Flash Sales', 'kera' );
    }
    ...
    {
        return ['slick', 'kera-custom-slick', 'jquery-countdowntimer'];
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'position_displayed',
            [
                'label'     => esc_html__('Position Displayed', 'kera'),
                'type'      => \Elementor\Controls_Manager::SELECT,
                'default'   => 'header',
                'options'   => [
                    'header'      => esc_html__('Header', 'kera'), 
                    'main'  => esc_html__('Main Content', 'kera'), 
                ],
            ]
        ); 
    ...
    private function register_control_header() {
        ...
        $this->add_control(
            ...
            [
                ...
                'options' => [
                    'text' => esc_html__('Text', 'kera'),
                    'image' => esc_html__('Image', 'kera')
                ]
            ]
        );  
        ...
        $this->add_control(
            $prefix .'text',
            [
                'label'     => esc_html__('Text', 'kera'),
                'type'      => Controls_Manager::TEXT,
                'default'   => esc_html__('Flash Sale', 'kera'),
                'conditions' => [
					'relation' => 'AND',
					'terms' => [
						[
        ...
        if (!empty($pages)) {
            $this->add_control(
                $prefix .'page',
                [
                    'label'        => esc_html__('Select Page', 'kera'),
                    'type'         => Controls_Manager::SELECT,
                    'options'      => $pages,
            ...
            $this->add_control(
                ...
                [
                    ...
                    'raw'             => sprintf(__('<strong>There are no pages in your site.</strong><br>Go to the <a href="%s" target="_blank">pages screen</a> to create one.', 'kera'), admin_url('edit.php?post_type=page')),
                    'separator'       => 'after',
    ...
    private function register_control_main() {
        ...
        $this->add_control(
            'date_title',
            [
                'label' => esc_html__('Title Date', 'kera'),
                'type' => Controls_Manager::TEXT,
                'default' => esc_html__('left', 'kera'),
                'label_block' => true,
                'condition' => [
                    'position_displayed' => 'main'
                ],
        ...
        $this->add_control(
            'date_title_ended',
            [
                'label' => esc_html__('Title deal ended', 'kera'),
                'type' => Controls_Manager::TEXT,
                'default' => esc_html__('Deal ended.', 'kera'),
                'label_block' => true,
                'condition' => [
                    'position_displayed' => 'main'
                ],
        ...
        $this->add_control(
            'end_date',
            [
                'label' => esc_html__('End Date', 'kera'),
                'type' => Controls_Manager::DATE_TIME,
                'label_block' => true,
                'placeholder' => esc_html__( 'Choose the end time', 'kera' ),
                'condition' => [
                    'position_displayed' => 'main'
                ],
            ]
        ...
        $this->add_control(
            'bg_top_flash_sale',
            [
                'label' => esc_html__('Background Top Flash Sale', 'kera'),
        ...
        $product_style = $this->get_template_product();
        unset($product_style['vertical-v1']);
        ...
        if (!empty($products)) {
            $this->add_control(
                $prefix .'products',
                [
                    'label'        => esc_html__('Products', 'kera'),
                    ...
                    'save_default' => true,
                    'label_block' => true,
                    'description' => esc_html__( 'Only search for sale products', 'kera' ),
                    'condition' => [
                        'position_displayed' => 'main'
                    ],
                ]
            ...
            $this->add_control(
                ...
                [
                    'type'            => Controls_Manager::RAW_HTML,
                    'raw'             => sprintf(__('You do not have any discount products. <br>Go to the <strong><a href="%s" target="_blank">Products screen</a></strong> to create one.', 'kera'), admin_url('edit.php?post_type=product')),
                    'separator'       => 'after',
                    'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
                    'condition' => [
                        'position_displayed' => 'main'
        ...
        $this->add_control(
            'enable_readmore',
            [
                'label' => esc_html__( 'Enable Button "Read More" ', 'kera' ),
    ...
    protected function register_control_viewall() {
        $this->start_controls_section(
            'section_readmore',
            [
                'label' => esc_html__( 'Read More Options', 'kera' ),
        ...
        $this->add_control(
            'readmore_text',
            [
                'label' => esc_html__('Button "Read More" Custom Text', 'kera'),
                'type' => Controls_Manager::TEXT,
    ...
    public function render_content_main() {
        ...
        if( count($ids) === 0 ) {
            echo '<div class="not-product-flash-sales">'. esc_html__('Please select the show product', 'kera')  .'</div>';
            return;
        }
...
$widgets_manager->register_widget_type(new Kera_Elementor_Product_Flash_Sales());